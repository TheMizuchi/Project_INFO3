...
public class TorcheView extends ViewElement {
	...
	public TorcheView (int x, int y, double radius, int win_w, int win_h, double scale, Torche t) {
		super(x, y, scale);
		this.torchModel = t;
	...
	public void setPosition (int x, int y, double radius, double scale) {
		this.scale = scale;
		this.radius = radius;
		this.x = (int) (x - this.radius*this.scale / 2);
		this.y = (int) (y - this.radius*this.scale / 2);
	}

	public void setPosition (int x, int y, double scale) {
		this.setPosition(x, y, this.radius, scale);
	...
	void paint (Graphics g) { // Ce paint doit être appelé en dernier, c'est la masque qui recouvre tous les éléments qui ne sont pas sensé être visible
		...
		Area mask_circle = new Area(new Ellipse2D.Double(this.x, this.y, this.radius*this.scale, this.radius*this.scale));
		...
		for (int i = 0; i < 40; i++) { // Dessine le fondu vers le noir au bord du champ de vision pour que la transtion soit plus progressive
			...
			g2d.drawOval(this.x + (40 - i) / 2, this.y + (40 - i) / 2, (int) (this.radius*this.scale) - 38 + i, (int) (this.radius*this.scale) - 38 + i);
		}
	}
