...
public class Hitbox {
	...
	public boolean contactEntity (double x, double y) {
		...
		edu.polytech.oop.collections.ICollection.Iterator it = list.iterator();
	...
	public boolean conflitHitbox (double x, double y, Hitbox hitb) {
		...
		return (this.pointInHitbox(hitx + dx, hity + dy) 
				|| this.pointInHitbox(hitxl + dx, hity + dy) 
				|| this.pointInHitbox(hitx + dx, hityh + dy) 
				|| this.pointInHitbox(hitxl + dx, hityh + dy) 
				|| hitb.pointInHitbox(m_x - dx, m_y - dy) 
				|| hitb.pointInHitbox(m_x - dx + m_largeur, m_y - dy) 
				|| hitb.pointInHitbox(m_x - dx, m_y + m_hauteur - dy) 
				|| hitb.pointInHitbox(m_x + m_largeur - dx, m_y + m_hauteur - dy));
	}
	...
	public boolean pointInHitbox (double x, double y) {
		...
		if (bool) {
			...
			System.out.println(x + " " + y + " est dans : " + m_x + " " + alpha + " " + m_y + " " + beta);
		} else {
